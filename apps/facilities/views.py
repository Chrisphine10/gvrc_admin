...
from .models import (
    ...
    ServiceCategory, OwnerType, Document, GbvCategory,
    ContactClick, DocumentType
)
from .serializers import (
    ...
    LoginSerializer, UserRegistrationSerializer, DocumentSerializer,
    GbvCategorySerializer, ContactClickSerializer, DocumentTypeSerializer
)
...
class FacilityListCreateView(generics.ListCreateAPIView):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'operational_status', 'ward', 'ward_detail__constituency',
        'ward_detail__constituency__county', 'is_active'
    ]
    search_fields = [
        'name', 'registration_number', 
        'ward_detail__ward_name', 'ward_detail__constituency__constituency_name',
        'ward_detail__constituency__county__county_name'
    ]
    ...
    def get_queryset(self):
        queryset = Facility.objects.select_related(
            'operational_status', 'ward_detail', 'ward_detail__constituency',
            'ward_detail__constituency__county', 'created_by', 'updated_by'
        ).prefetch_related(
            'contacts_new__contact_type',
            'coordinates',
            'services_new__service_category',
            'owners_new__owner_type',
            'users'
        )
        return queryset
...
class FacilitySearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = Facility.objects.select_related(
            'operational_status', 'ward_detail', 'ward_detail__constituency', 'ward_detail__constituency__county'
        ...
        if county:
            queryset = queryset.filter(ward_detail__constituency__county_id=county)
        
        if constituency:
            queryset = queryset.filter(ward_detail__constituency_id=constituency)
        
        if ward:
            queryset = queryset.filter(ward_detail_id=ward)
        
        if service_category:
            queryset = queryset.filter(services_new__service_category_id=service_category).distinct()
...
def dashboard_stats(request):
    ...
    stats = {
        ...
        'facilities_by_county': list(
            ...
            .select_related('ward_detail__constituency__county')
            .values('ward_detail__constituency__county__county_name')
            .annotate(count=Count('id'))
            .order_by('-count')[:10]
        ),
        'recent_facilities': FacilityListSerializer(
            Facility.objects.filter(is_active=True)
            .select_related('operational_status', 'ward_detail__constituency__county')
...
def nearby_facilities(request):
    ...
    facilities = Facility.objects.filter(
        ...
        'operational_status', 'ward_detail__constituency__county', 'coordinates'
    ).prefetch_related(
        'contacts_new__contact_type', 'services_new__service_category'
    )
...
# GBV Views
class GbvCategoryListCreateView(generics.ListCreateAPIView):
    queryset = GbvCategory.objects.all()
    serializer_class = GbvCategorySerializer
    permission_classes = [IsAuthenticated]


class GbvCategoryDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = GbvCategory.objects.all()
    serializer_class = GbvCategorySerializer
    permission_classes = [IsAuthenticated]
    lookup_field = 'gbv_category_id'
...
# Contact Click Views
class ContactClickListCreateView(generics.ListCreateAPIView):
    queryset = ContactClick.objects.all()
    serializer_class = ContactClickSerializer
    permission_classes = [IsAuthenticated]


class ContactClickDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = ContactClick.objects.all()
    serializer_class = ContactClickSerializer
    permission_classes = [IsAuthenticated]
    lookup_field = 'click_id'
