...
- `POST /mobile/chat/start/` - Start or retrieve a conversation for the device (manual)
- `GET /mobile/chat/get-or-create/` - **NEW**: Auto-create conversation without topic input
...
- `POST /mobile/chat/auto/send-message/` - **NEW**: Send message with auto-conversation creation
...
**Automatic Chat Features:**
- Conversations are auto-created with smart subjects based on content and time
- No need to manually specify topics - subjects are generated automatically  
- Seamless chat experience without explicit conversation creation
